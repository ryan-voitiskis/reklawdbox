...
Lightweight prompts that use existing tools in creative ways. No new tools required — the agent combines search, scoring, analysis, and its own musical knowledge to deliver value.

<Aside type="note">
  These prompts work best when cache coverage is high. Run `cache_coverage` first.
</Aside>
...
3. Identify specific gaps:
   ...
   - Labels you buy from heavily that have releases you're missing
...
<Aside type="tip">
  This works well as a conversation — tell the agent what you found, what you liked, what missed, and it refines its suggestions.
</Aside>
...
Analyze what you actually played and learn from it. Requires [session history tools](/concepts/architecture/) (see issue #24).
...
<Aside type="caution">
  This prompt depends on session history tools (issue #24) which are not yet implemented.
</Aside>
...
3. Plan a key sequence based on the requested style:
   - **Rising tension:** move clockwise around the wheel (6A → 7A → 8A → ...), each step adds energy
   - **Major-minor shift:** move between inner and outer ring (6A → 6B → 7B → 7A) for emotional contrast
   - **Full rotation:** visit all 12 positions and return to start — a complete harmonic journey
