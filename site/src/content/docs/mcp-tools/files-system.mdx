...
Scan for tracks with missing audio files on disk. Optionally suggests relocated files by matching filenames across content roots.

| Parameter | Type | Required | Description |
|-----------|------|:--------:|-------------|
| `path_prefix` | string | | Scope to tracks whose file path starts with this prefix |
| `suggest_relocations` | boolean | | Attempt fuzzy filename matching for relocations (default: `true`) |
