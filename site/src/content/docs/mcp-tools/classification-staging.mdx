...
Auto-fill missing years (year=0) from Discogs enrichment. Stages non-conflicting years automatically; reports conflicts where the current year disagrees with enrichment.
...
| `fields` | string[] | | Fields to unstage: `"genre"`, `"comments"`, `"rating"`, `"color"`, `"label"`, `"year"` (omit to clear all fields on matched tracks) |
