...
Before export, reklawdbox checks for a backup script (in order): `REKLAWDBOX_BACKUP_SCRIPT` env var, then built-in probe paths. If found, it runs synchronously with `--pre-op` before writing. Failures restore staged changes and block export with an error.
