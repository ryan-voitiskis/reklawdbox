...
Ask the user where loose tracks should go. Albums have a clear convention (`Artist/Album (Year)/`), but loose track organization varies by user — some use flat directories, source-based folders, or other schemes.

Once the user specifies a destination, move all loose tracks there:

```sh
mkdir -p "/path/to/dest/loose-tracks-dir"
mv -n "/path/to/batch/Artist - Title.wav" "/path/to/dest/loose-tracks-dir/"
```
