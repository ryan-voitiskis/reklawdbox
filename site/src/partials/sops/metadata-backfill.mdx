...
- **`backfill_labels` is enrichment-dependent.** The tool pulls labels from Discogs, MusicBrainz, and Bandcamp enrichment caches — it does not check file tags or the web. Tracks without cached enrichment are skipped (unless `auto_enrich=true`). For tracks still unlabeled after the tool runs, the agent researches labels manually (step 1c).
- **Years use a priority cascade.** `backfill_years` tries multiple sources in order: file tags → folder path `(YYYY)` → Discogs enrichment → MusicBrainz enrichment → Bandcamp enrichment. For tracks still missing years after the tool runs, the agent researches release dates (step 2c).
...
Check the reported baseline for unlabeled tracks:

```
cache_coverage(has_label=false)
```

`cache_coverage` reports scoped Stratum, Essentia, and Discogs entries. For
Discogs, use `searched_percent` to find unattempted work and `has_result` to
understand usable matches. Searched includes legitimate no-matches; errors
count as neither searched nor matched and remain retryable. Essentia is useful
evidence but not a prerequisite for metadata backfill.
...
2. **Bandcamp** is a targeted source for underground and independent releases.
Use the tool-specific gaps from the backfill response to select work:

```
enrich_tracks(has_label=false, skip_cached=true, providers=["bandcamp"], max_tracks=50)
```

3. **MusicBrainz** supplies release-group and label fallbacks and is fetched by
the relevant `auto_enrich` path.

Run targeted enrichment for actual gaps, retry errors, and accept a cached
no-match as a completed search. Do not require every provider to match every
track, and do not invent percentages for targeted sources.

For every bounded `enrich_tracks` scope above, begin at `offset=0`, record
successes and identity-bearing failures, and continue with
`offset=page.next_offset` while `page.has_more` is true. Cached searches do not
consume work slots. After traversing the scope, retry failures with explicit
`track_ids`, then re-run the relevant `cache_coverage`/backfill readiness check.
Stop only when the needed searched coverage is satisfied or unresolved work is
reported. These offsets are not durable across library or selector-order
changes; restart at zero after either changes. Keep `providers`, `skip_cached`,
and `force_refresh` fixed for a traversal. Changing provider scope or cache
policy can make earlier indices pending and requires a restart at zero.

**Alternative: skip manual hydration** and use `auto_enrich=true` on the
backfill tools (see steps 1 and 2). Label and year backfill fetch Bandcamp and
MusicBrainz for their exact unresolved provider gaps; album backfill fetches
Bandcamp. This is slower for large collections but requires no manual batching.

Whenever `auto_enrich=true`, inspect the additive `auto_enrichment` report
before treating enrichment as complete:

- `auto_enriched` and `auto_enriched_by_provider` retain their legacy meaning:
  successful provider matches, even if a later cache write fails.
- `cache_writes_succeeded` is the count of acknowledged durable positive and
  negative cache rows. A completed no-match is a successful durable write.
- If `operation_failed=true`, inspect `lookup_failed`,
  `cache_writes_failed`, `serialization_failed`, `worker_failed`,
  `writer_failed`, and the typed provider/key `failures` before deciding what
  to retry. Failure details are capped at 50; `failures_truncated=true` means
  additional failures occurred.
- Successfully persisted rows are always re-scanned and can contribute staged
  suggestions even when another provider/key fails. Retry only the failed
  identities; do not discard the durable partial success.
- `dry_run=true` suppresses metadata staging only. Explicit auto-enrichment may
  still persist Reklawdbox-owned provider cache rows before the re-scan.
...
Or, to automatically fetch MusicBrainz and Bandcamp data for their uncached
label gaps:
...
The tool scans the entire collection (excluding samples), looks up each track's enrichment across Discogs, MusicBrainz, and Bandcamp, and:
...
Label precedence remains Discogs → MusicBrainz → Bandcamp. The additive
`auto_enriched_by_provider` counts successful MusicBrainz and Bandcamp matches;
their sum equals `auto_enriched`. These are provider-match counters, not
durability counters. Use `auto_enrichment.cache_writes_succeeded` for durable
positive/no-match rows. Provider errors remain retryable and do not write
durable cache rows, while completed no-matches are cached. If some keys fail,
durable successes are still re-scanned with Discogs → MusicBrainz → Bandcamp
precedence and may be staged.

The summary includes `no_enrichment_by_provider` for all scanned tracks that
lack a usable enrichment label from each provider; these counts are broader
than the remaining unlabeled set. Use `research_queue` for the actual unresolved
label workload. If `no_bandcamp` is high, run targeted Bandcamp enrichment and
re-run.
...
The mutating/auto-enriching call above is the one and only side-effecting pass.
If `conflict_page.has_more` is true, collect every later conflict page without
repeating those effects:

```
backfill_labels(dry_run=true, auto_enrich=false, conflict_offset=CONFLICT_NEXT_OFFSET)
```

Set `CONFLICT_NEXT_OFFSET` from the prior `conflict_page.next_offset` and repeat
only while `has_more` is true. `conflicts_truncated=true` is the legacy alias
for that condition. Keep the library unchanged during retrieval or restart the
conflict traversal at zero.
...
3. **Research labels** using web search, store lookups (`lookup_discogs`, `lookup_bandcamp`, `lookup_musicbrainz`), label catalogs, and model knowledge.
...
Before treating this provider pass as complete, inspect
`auto_enrichment.operation_failed` and its failure counters. Provider matches
remain counted by legacy `auto_enriched`; acknowledged positive and negative
rows are counted by `cache_writes_succeeded`. Durable successes are re-scanned
through the unchanged year cascade even when another key needs a retry.

The tool scans the entire collection (excluding samples) and tries five sources in priority order for each track with year=0:
...
4. **MusicBrainz enrichment** — falls back to the cached MusicBrainz `first-release-date`.
5. **Bandcamp enrichment** — falls back to the cached Bandcamp `release_date`. Particularly effective for underground/independent electronic music.

The first source to produce a valid year (1900–2099) wins. Non-conflicting fills are auto-staged. For tracks that already have a non-zero year, the tool checks Discogs enrichment only — if the Discogs year differs from the current year, the track is reported as a conflict. MusicBrainz and Bandcamp are not compared against existing years.

The summary includes `remaining_uncached_providers` showing which targeted
sources are missing data for remaining year-zero tracks. Use those gaps to
choose enrichment, then re-run (or use `auto_enrich=true`).

Report the summary: "Year backfill complete: N years staged (N file tags, N folder paths, N Discogs, N MusicBrainz, N Bandcamp), N already correct, N conflicts, N without any source." Then immediately proceed to resolve conflicts.
...
Run this for the selected Bandcamp gaps, then **re-run `backfill_years()`** to
pick up new data. A legitimate no-match can remain unresolved and move to web
research; it does not need to become a provider match.

Traverse the Bandcamp call via `page.next_offset`/`page.has_more`, retain
failures, and retry those explicit IDs only after the full scope traversal.
...
2. **Search for release dates** — use web search, Discogs links, store lookups (`lookup_discogs`, `lookup_bandcamp`, `lookup_musicbrainz`), label catalogs, and model knowledge to find the original release year.
...
Album auto-enrichment is Bandcamp-only. Check the additive
`auto_enrichment` report before treating a match as durable; persisted partial
successes are re-scanned and staged while typed failures identify any keys that
need retry. Album gaps remain best-effort and do not become a new export gate.
