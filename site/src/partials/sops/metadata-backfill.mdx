...
`cache_coverage` reports scoped Stratum, Essentia, Discogs, and Beatport entries.
For Discogs and Beatport, use `searched_percent` to find unattempted work and
`has_result` to understand usable matches. Searched includes legitimate
no-matches; errors count as neither searched nor matched and remain retryable.
Essentia is useful evidence but not a prerequisite for metadata backfill.
...
2. **Beatport** (label data extracted from search results since v0.15; older cache entries lack labels):

```
enrich_tracks(has_label=false, skip_cached=true, providers=["beatport"], max_tracks=50)
```

If Beatport `searched_percent` is complete but labels are still missing, existing
cache entries may predate label extraction. Re-enrich with `force_refresh=true`
to populate them.

3. **Bandcamp** is a targeted source for underground and independent releases.
...
4. **MusicBrainz** is an individual lookup used during year research (step 2c)
or fetched by the relevant `auto_enrich` path.
