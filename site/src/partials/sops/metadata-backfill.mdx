...
- **Manual research is mandatory (labels and years).** Steps 3 and 6 must be completed before proceeding to export. The automated backfill tools handle the bulk, but tracks left without labels or years afterward must be individually researched via web search, store lookups, and model knowledge. Do not proceed to step 8 (export) until steps 3 and 6 are done. Skipping research and exporting only the automated results leaves the job half-finished.
...
4. **MusicBrainz** is hydrated automatically by `lookup_musicbrainz` during year research (step 6) or via `auto_enrich`.
...
### 3. Research remaining label gaps (mandatory)

After steps 1–2, tracks may still lack labels. The automated backfill checks four enrichment providers (Discogs, MusicBrainz, Bandcamp, Beatport). This step fills the rest using web research and model knowledge. **Do not skip this step or defer it to a later export — complete it before proceeding to step 4.**
...
### 6. Research remaining year gaps (mandatory)
...
**Pre-export gate — do not skip.** Before exporting, confirm all research steps are complete:
- Step 3 (label research): all unlabeled tracks have been researched. Remaining gaps are genuinely unresolvable (private edits, anonymous tracks), not just unattempted.
- Step 6 (year research): all year-zero tracks have been researched. Same standard.
