...
### 3. Research remaining label gaps (mandatory — enforced by write_xml gate)

After steps 1–2, tracks may still lack labels. The `backfill_labels` output includes a `research_queue` section listing the count and top artists by frequency. **`write_xml` will refuse to export until this step is done** — it checks whether `backfill_labels` reported unlabeled tracks and blocks export unless `skip_label_gate=true` is passed.

Start by fetching the first batch of unlabeled tracks:

```
search_tracks(has_label=false, max_tracks=50)
```
