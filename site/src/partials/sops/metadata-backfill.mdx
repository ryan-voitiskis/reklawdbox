...
For each conflict, stage the **lowest (earliest) year** from either the current value or the enrichment value via `update_tracks`. The earliest year best approximates the track's production era, which is the signal used for genre classification.
...
Report the summary: "Year conflicts resolved: N auto-staged (used earliest year), N already had the earliest year, N flagged for review."

### 6. Research remaining year gaps

After `backfill_years`, some tracks will still have year=0 with no source data. The tool reports these as `remaining_year_zero`. The agent must research every one of them — this is not optional.
