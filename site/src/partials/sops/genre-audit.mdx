...
when coverage is incomplete; missing evidence may lower confidence, but it can
also appear as flags on high- or medium-confidence results. Inspect those flags
before bulk approval and review tracks without independent supporting evidence
individually. Retry actual errors, but do not require a match for a legitimately
searched release.
