...
- Use `lookup_discogs()` / `lookup_bandcamp()` / `lookup_musicbrainz()` for missing album/year
...
4. If the drift is a genuinely different name (not just character substitution), look up via `lookup_discogs` / `lookup_bandcamp` / `lookup_musicbrainz`
