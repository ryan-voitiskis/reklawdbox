...
Only when both `total_open` = 0 and `failed_reads` = 0 → skip to Final Report.
...
Present: scope, files scanned, `failed_reads`, pass rate, fixes by type, deferred items, and next steps (Rekordbox import, genre classification SOP). Declare the audit clean only when `total_open` = 0 and `failed_reads` = 0; otherwise label it an incomplete audit and list the unreadable paths.
