...
| M30 | ~~`serde(untagged)` enums in `tags.rs` make deser errors opaque~~ N/A — these enums are serialize-only (no `Deserialize` derive); untagged deser errors cannot occur at runtime | |
