...
#[allow(dead_code)]
#[cfg(not(test))]
pub(crate) fn load_rekordbox_grid_for_path(file_path: &str) -> Option<stratum_dsp::BeatGrid> {
    let db_path = crate::adapters::rekordbox::resolve_db_path()?;
    let conn = crate::adapters::rekordbox::open(&db_path).ok()?;
    load_rekordbox_grid_for_path_with_conn(&conn, file_path)
}

/// Unit tests never inspect the user's Rekordbox library as a side effect.
#[allow(dead_code)]
#[cfg(test)]
pub(crate) fn load_rekordbox_grid_for_path(_file_path: &str) -> Option<stratum_dsp::BeatGrid> {
    None
}

pub(crate) fn load_rekordbox_grid_for_path_with_conn(
    ...
    let abs = match resolve_anlz_path(&analysis_data_path) {
        Some(p) => p,
        None => {
            tracing::warn!(
    ...
    match read_beat_grid(std::path::Path::new(&abs)) {
        Ok(g) => Some(g),
        Err(e) => {
            tracing::warn!(
