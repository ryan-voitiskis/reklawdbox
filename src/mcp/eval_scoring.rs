...
mod tests {
    ...
    use crate::mcp::planning::scoring::*;
    ...
    fn eval_describe_pool_optimal_bpm_sweep() {
        ...
        let result = crate::mcp::planning::pools::sweep_optimal_reference_bpm(&profiles, &bpms);
    ...
    fn dummy_norm_stats(dims: usize) -> crate::store::TimbralNormStats {
        crate::store::TimbralNormStats {
            ...
            sample_count: 100,
            source_fingerprint: "eval-dummy-source".to_string(),
    ...
    fn open_timbral_norm_test_store() -> (tempfile::TempDir, Connection) {
        ...
        let conn = crate::store::open(store_path.to_str().unwrap()).unwrap();
        (dir, conn)
    }
    ...
    fn cache_timbral_test_entry(conn: &Connection, file_path: &str, features_json: &str) {
        let identity = super::super::analysis::audio_cache_identity(file_path).unwrap();
    ...
    fn eval_normalize_timbral_vector_zscore() {
        let stats = crate::store::TimbralNormStats {
            ...
            sample_count: 50,
            source_fingerprint: "eval-zscore-source".to_string(),
            analysis_version: crate::audio::ESSENTIA_SCHEMA_VERSION.to_string(),
            vector_schema_version: TIMBRAL_VECTOR_SCHEMA_VERSION.to_string(),
        };
    ...
    fn eval_normalize_timbral_vector_dimension_mismatch() {
        let stats = crate::store::TimbralNormStats {
            dims: vec![(0.0, 1.0); 3],
            sample_count: 50,
            source_fingerprint: "eval-dimension-source".to_string(),
            analysis_version: crate::audio::ESSENTIA_SCHEMA_VERSION.to_string(),
            vector_schema_version: TIMBRAL_VECTOR_SCHEMA_VERSION.to_string(),
        };
        assert!(
    ...
    fn timbral_source_snapshot_excludes_unusable_sources_without_changing_digest() {
        ...
        let old_identity = super::super::analysis::audio_cache_identity(&old_schema).unwrap();
        crate::store::set_audio_analysis(
            &conn,
    ...
    fn timbral_norm_compute_uses_one_exact_fresh_snapshot() {
        ...
        assert_eq!(
            crate::store::get_timbral_norm_stats(&conn)
                .unwrap()
                .unwrap(),
            stats
        );
    ...
    fn timbral_norm_ensure_rebuilds_mismatched_or_incoherent_stats() {
        ...
        crate::store::save_timbral_norm_stats(&conn, &wrong_dimensions).unwrap();
        ...
        crate::store::save_timbral_norm_stats(&conn, &wrong_count).unwrap();
    ...
    fn timbral_norm_legacy_rows_invalidate_and_recompute_once() {
        ...
        let conn = crate::store::open(store_path.to_str().unwrap()).unwrap();
        let legacy = crate::store::get_timbral_norm_stats(&conn)
            .unwrap()
            .unwrap();
