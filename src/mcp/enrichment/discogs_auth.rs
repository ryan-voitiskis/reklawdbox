...
#[cfg(test)]
use super::core::take_test_discogs_lookup_override;
...
#[cfg(test)]
#[derive(Default)]
pub(in crate::mcp) struct InMemoryDiscogsSessionPersistence {
    state: Mutex<InMemoryDiscogsSessionState>,
}

#[cfg(test)]
impl InMemoryDiscogsSessionPersistence {
    pub(in crate::mcp) fn set_session(
        &self,
        broker_url: &str,
        session_token: &str,
        expires_at: i64,
    ...
    pub(in crate::mcp) fn has_session(&self, broker_url: &str) -> bool {
        self.state
            .lock()
            .expect("in-memory Discogs session state should not be poisoned")
            .sessions
    ...
    pub(in crate::mcp) fn session_matches(&self, broker_url: &str, expected_token: &str) -> bool {
        self.state
            .lock()
            .expect("in-memory Discogs session state should not be poisoned")
            .sessions
    ...
    pub(in crate::mcp) fn store_count(&self) -> usize {
        self.state
            .lock()
            .expect("in-memory Discogs session state should not be poisoned")
            .store_count
    ...
    pub(in crate::mcp) fn clear_count(&self) -> usize {
        self.state
            .lock()
            .expect("in-memory Discogs session state should not be poisoned")
            .clear_count
    ...
    pub(in crate::mcp) fn fail_next_store(&self) {
        self.state
            .lock()
            .expect("in-memory Discogs session state should not be poisoned")
    ...
    pub(in crate::mcp) fn fail_next_clear(&self) {
        self.state
            .lock()
            .expect("in-memory Discogs session state should not be poisoned")
...
#[cfg(test)]
#[derive(Clone)]
pub(in crate::mcp) struct DiscogsAuthTestDependencies {
...
impl DiscogsAuthTestDependencies {
    ...
    pub(in crate::mcp) fn with_entry_barrier(mut self, barrier: Arc<tokio::sync::Barrier>) -> Self {
...
impl ReklawdboxServer {
    pub(in crate::mcp) fn set_discogs_auth_test_dependencies(
        &self,
...
impl DiscogsAuthGateway for ServerDiscogsAuthGateway<'_> {
    ...
    async fn pending_status(
        ...
        discogs::device_session_status(&self.server.context.enrichment.http, cfg, pending)
            .await
    ...
    async fn finalize_pending(
        ...
        discogs::device_session_finalize(&self.server.context.enrichment.http, cfg, pending).await
    }
    ...
    async fn start_pending(
        ...
        discogs::device_session_start(&self.server.context.enrichment.http, cfg).await
    }
}
...
async fn resolve_auth_transition(
    ...
    let _transition = server.context.enrichment.discogs_auth_lock.lock().await;
    resolve_auth_transition_locked(server, cfg, now, persistence).await
}

#[cfg(test)]
pub(in crate::mcp) async fn resolve_discogs_auth_transition_for_test(
    server: &ReklawdboxServer,
...
pub(in crate::mcp) async fn lookup_discogs_remote(
    server: &ReklawdboxServer,
    artist: &str,
    title: &str,
    album: Option<&str>,
