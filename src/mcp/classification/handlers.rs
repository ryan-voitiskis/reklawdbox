...
pub(in crate::mcp) fn handle_classify_tracks(
    ...
    let (results, cache_errors) = {
        ...
        classification::classify_batch(&store_conn, &tracks, &overrides).map_err(cache_error)?
    };
    ...
    if let Some(ref levels) = params.auto_stage {
        ...
        let (staged, total_pending) = server.context.mutation.changes.stage(track_changes);
...
pub(in crate::mcp) fn handle_audit_genres(
    server: &ReklawdboxServer,
    ...
    let (results, cache_errors) = {
        let store_conn = server.cache_store_conn()?;
        classification::classify_batch(&store_conn, &tracks, &[]).map_err(cache_error)?
    };
...
pub(in crate::mcp) fn build_genre_distribution(
...
pub(in crate::mcp) fn handle_calibrate_audio_profiles(
    server: &ReklawdboxServer,
...
pub(in crate::mcp) fn handle_calibration_coverage(
    server: &ReklawdboxServer,
