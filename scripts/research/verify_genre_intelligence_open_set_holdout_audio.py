#!/usr/bin/env python3
"""Verify decoded-audio isolation for the sealed Plan 071/072 holdout."""
...
def manifest_paths(path: Path, expected_sha256: str, expected_rows: int) -> list[str]:
    if evaluation.sha256_file(path) != expected_sha256:
    ...
    if len(set(paths)) != len(paths) or any(
        not Path(value).is_file() for value in paths
    ):
