#!/usr/bin/env python3
"""Prepare prediction-blind review batches for the Plan 073 holdout offers."""
...
EXPERIMENT_PREFIX = "genre-intelligence-precision-first-v1-h"
...
def stable_order(row_id: str) -> str:
    return hashlib.sha256(
        f"genre-intelligence-precision-first-review-v1|{row_id}".encode("utf-8")
    ).hexdigest()
...
def offered_positions(predictions: dict[str, Any]) -> list[int]:
    ...
    for position, row in enumerate(rows, start=1):
        expected_id = f"GIP-{position:03d}"
...
def review_rows(
    ...
    ordered = sorted(positions, key=lambda value: stable_order(f"GIP-{value:03d}"))
    result = []
        ...
        result.append(
            {
                ...
                "source_row_id_private": f"GIP-{position:03d}",
            }
        )
...
def mapping_for_batch(rows: list[dict[str, Any]], batch_number: int) -> dict[str, Any]:
    batch_label = f"P{batch_number:02d}"
    selected = []
    for position, row in enumerate(rows, start=1):
        selected.append(
            {
    ...
    return {
        ...
        "method_status": "blind_precision_first_holdout_review_pending",
...
def main() -> int:
    ...
    for batch_number, batch_rows in enumerate(batches(rows), start=1):
        ...
        path = args.output_dir / f"precision-first-review-p{batch_number:02d}-mapping.json"
        corpus.atomic_write(
            path,
    ...
    manifest = {
        "schema_version": 1,
        "experiment_id": "genre-intelligence-precision-first-review-v1",
