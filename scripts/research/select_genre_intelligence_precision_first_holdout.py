#!/usr/bin/env python3
"""Seal the powered Plan 073 precision-first holdout."""
...
METHOD_STATUS = "sealed_after_model_freeze_before_new_holdout_inference"
SEED = "genre-intelligence-precision-first-holdout-v1"
HOLDOUT_ROWS = 150
EXPECTED_HASHES = {
...
def eligible_rows(
    ...
    for track in tracks:
        ...
        artist = library.normalized(str(track["artist"]))
        release = library.release_group(track)
        stratum = library.broad_target(str(track["current_genre"]))
        if not path or path in seen_paths:
            exclusions["blank_or_duplicate_path"] += 1
            continue
        seen_paths.add(path)
...
def select_holdout(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        by_stratum[str(row["broad_sampling_stratum"])].append(row)
    for stratum, values in by_stratum.items():
        values.sort(
            key=lambda row: stable_hash(
                "quota", stratum, row["track_id"], row["file_path"]
            )
        )
    ...
    def add(row: dict[str, Any]) -> bool:
        ...
        counts[str(row["broad_sampling_stratum"])] += 1
        return True
    ...
    if len(selected) != HOLDOUT_ROWS:
        raise ValueError(
            f"precision-first selector produced {len(selected)} rows; "
...
def validate_hash(path: Path, field: str) -> str:
    ...
    if value != EXPECTED_HASHES[field]:
        raise ValueError(f"{field} differs from the frozen Plan 073 input")
    return value
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    observed = {
        ...
        "first_consumed_holdout_sha256": validate_hash(
            args.first_consumed_holdout, "first_consumed_holdout_sha256"
        ),
        "second_consumed_holdout_sha256": validate_hash(
            args.second_consumed_holdout, "second_consumed_holdout_sha256"
        ),
        "frozen_development_result_sha256": validate_hash(
            args.frozen_development_result, "frozen_development_result_sha256"
        ),
        "frozen_model_sha256": validate_hash(
            args.frozen_model, "frozen_model_sha256"
        ),
    ...
    excluded_paths, excluded_artists, excluded_releases = (
        library.development_identities(
            tracks_by_path,
            playlists,
            [development],
            exclusion_playlists,
    ...
    eligible, exclusions = eligible_rows(
        tracks, excluded_paths, excluded_artists, excluded_releases
    )
    selected = [
    ...
    return {
        ...
        "selection_rule": {
            ...
            "development_and_both_consumed_holdout_groups_excluded": True,
...
def main() -> int:
    ...
    parser.add_argument("--frozen-development-result", required=True, type=Path)
